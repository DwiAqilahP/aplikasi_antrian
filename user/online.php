...
<body>
    ...
    <div class="main-content">     
        <main>
            ...
            <div class="page-header">
                <h1>Sistem Antrian <?php echo $tmpl['nama_pelayanan'] ?> Secara Online</h1>
            </div>
            <?php } ?>
    ...
    ?>
    <form method="POST" action="proses.php" style="color: white">
      <h5>Lengkapi formulir ini sesuai dengan KTP anda!</h5>
        ...
        <button type="submit" class="btn btn-primary" id="insert" onclick="play()">Simpan</button>
    </form>
        </main> 
    </div>
    ...
    if (isset($_POST['submit'])) { ?>
      ...
      <?php
            include "../config/database.php";
            $id_antrian = mysqli_query($conn, "SELECT id_antrian FROM antrian WHERE id_pelayanan = '$id_pelayanan' ORDER BY id_antrian DESC LIMIT 1");
            $tampil = mysqli_fetch_assoc($id_antrian);
            $id_antrian = $tampil['id_antrian'];
            $nama = $_POST['nama'];
            $alamat = $_POST['alamat'];
            $nik = $_POST['nik'];
